...
import { ScriptEntity, ScriptInfoArray } from "../types";
import { CONFIG_SCRIPTSERVICE } from '../config';
import * as https from 'https';

export interface IScriptService {
    getAllScriptInfoAsync(): Promise<ScriptInfoArray>;
}
...
export class ScriptService implements IScriptService {
    ...
    private async fetchAndCheckAsync<T>(endpoint: string, method: https.RequestOptions["method"], errorMessagePrefix: string, body?: object): Promise<T> {
    ...
    public async getAllScriptInfoAsync(): Promise<ScriptInfoArray> {
        return await this.fetchAndCheckAsync<ScriptInfoArray>(CONFIG_SCRIPTSERVICE.SCRIPT_ENDPOINT_URI, 'GET', 'Failed to get script info');
    }

    public async getScriptEntityAsync(uniqueIdentifier: string): Promise<ScriptEntity> {
        return await this.fetchAndCheckAsync<ScriptEntity>(`${CONFIG_SCRIPTSERVICE.SCRIPT_ENDPOINT_URI}${uniqueIdentifier}`, 'GET', 'Failed to get script entity');
    }
}
